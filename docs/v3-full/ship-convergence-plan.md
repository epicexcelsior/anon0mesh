...
**Phase 3.5 — Send + Receive polish:** complete with documented placeholders. Family-wallet-aligned receive (stealth/standard segmented control with purple-themed QR when stealth selected, circular copy+share actions with icon morph + cyan pulse and no toast) and send recipient (single-row paste pill, full-width Scan QR row, nearby peers section, dev-only mock devnet fill). Send amount + review screens stripped of verbose copy and visible dividers — review is now amount hero + three summary rows + compact stealth toggle + SlideToConfirm. Token primitives (`TokenLogo` for SOL/USDC brand marks) and hooks (`useHideBalance` with shared context) landed as reusable foundation. Decorative dot-dash `traceRow` stripped from every scaffold and hero card across the codebase. Stealth crypto wiring lives in `worktrees/anon0mesh-fork-ui/lib/stealth/` and hooks up in Phase 7.

**Phase 3.5 known placeholders / deferred items:**

- **`SwipeDismissView` primitive exists but NOT applied to receive.** Wrapping the full receive screen in GestureDetector blocked native Pressable taps on Android (X / Copy / Share / Stealth segmented control stopped responding). The primitive stays in `components/primitives/` for a future pass that uses `.simultaneousWithExternalGesture()` or a drag-handle-only pattern — likely during Phase 6 peers sheet rebuild when we can design around the gesture from scratch.
- **Stealth address on receive is a PREVIEW string,** not a real meta-address. Purple QR + purple pill label it clearly as "Preview — stealth wiring lands in Phase 7." Real derivation uses `worktrees/anon0mesh-fork-ui/lib/stealth/StealthAddressGenerator.ts`.
- **Send amount screen has no token picker yet** — defaults to SOL only. User needs SOL + USDC toggle with TokenLogo (both logos exist as primitives) and support for fine-grained decimal input beyond the current keypad's 2-decimal cap. Add during Phase 7 when token list becomes multi-asset.
- **Send review screen is functional but visually lighter than Family reference.** SlideToConfirm height unchanged in this phase. Family-quality polish deferred until transport truth (Phase 7) so we aren't polishing a screen that will shift content.
- **Send success screen untouched** this phase — still has verbose "Transfer settled" / "Transfer in motion" copy + explanatory subtitle. Quick trim candidate for a follow-up pass.
