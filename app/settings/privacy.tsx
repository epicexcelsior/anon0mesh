...
export default function PrivacyScreen() {
  ...
  const { privacy, loading, updatePrivacy } = usePreferences();
  ...
  function cyclePrivacyMode() {
    const idx = PRIVACY_MODES.indexOf(privacy.privacyMode);
    void updatePrivacy({ privacyMode: PRIVACY_MODES[(idx + 1) % PRIVACY_MODES.length] });
    haptics.tap();
    sound.buttonTap();
  }

  function cycleRotationCadence() {
    const idx = ROTATION_CADENCES.indexOf(privacy.rotationCadence);
    void updatePrivacy({ rotationCadence: ROTATION_CADENCES[(idx + 1) % ROTATION_CADENCES.length] });
    haptics.tap();
    sound.buttonTap();
  }
    ...
    <View style={styles.root}>
      ...
      >
        ...
        <View style={styles.section}>
          ...
          <GlassSurface variant="regular" style={styles.card}>
            <TouchableOpacity
              accessibilityLabel={`Transaction privacy mode: ${privacy.privacyMode}. Tap to change.`}
            ...
            >
              ...
              <View style={styles.rightWrap}>
                <Text style={styles.valueText}>{privacy.privacyMode}</Text>
        ...
        <View style={styles.section}>
          ...
          <GlassSurface variant="regular" style={styles.card}>
            <TouchableOpacity
              accessibilityLabel={`Key rotation cadence: ${privacy.rotationCadence}. Tap to change.`}
              accessibilityRole="button"
              style={styles.row}
            ...
            >
              ...
              <View style={styles.rightWrap}>
                <Text style={styles.valueText}>{privacy.rotationCadence}</Text>
                <Icon name="chevron-right" size={16} color={theme.colors.textMuted} />
              </View>
            </TouchableOpacity>
          </GlassSurface>
